...
def run_glow_core(ctx: InputContext, provider: str = "gemini", api_key: str = "", memory: Optional[MemoryStore] = None) -> DecisionPack:
    ...
    if not allowed:
        pack = DecisionPack(
            ...
            root_causes=["Không hỗ trợ mục tiêu có rủi ro gây hại/phi pháp."],
            ...
            meta={"ts": ts, "engine": "glowcore_v1"},
        )
        memory.append({"ts": ts, "engine_used": pack.engine_used, "mode": pack.mode, "input": ctx.__dict__})
        return pack
